...
def get_tomorrows_races(debug=False):
    """Scrape Sporting Life 'tomorrow' racecards for upcoming races."""
    base_url = "https://www.sportinglife.com/racing/racecards/tomorrow"
    url_rows = []
    ...
    try:
        ...
        print(f"[get_tomorrows_races] Error fetching {base_url}: {e}")
        return pd.DataFrame(columns=["Date", "Location", "Time", "prerace_URL", "postrace_URL", "Status"])
    ...
    if debug:
        print(f"[get_tomorrows_races] Found {len(meetings)} meetings for tomorrow")
    ...
    for meeting in meetings:
        ...
        race_containers = meeting.find_all("div", attrs={"data-test-id": "race-container"})
        if debug:
            print(f"[get_tomorrows_races] {location}: {len(race_containers)} races found")
        ...
        for race in race_containers:
            try:
                ...
                if "/racecards/" not in relative_url:
                    continue

                prerace_url = "https://www.sportinglife.com" + relative_url
                postrace_url = prerace_url.replace("/racecards/", "/results/")
                ...
                if debug:
                    print(f"[get_tomorrows_races] Error parsing race: {e}")
                continue

    return pd.DataFrame(url_rows)
...
if __name__ == "__main__":
    ...
    results_df = get_race_urls(yesterday, debug=True)

    print(f"🏇 Fetching racecards for tomorrow ...")
    tomorrow_df = get_tomorrows_races(debug=True)

    combined_df = pd.concat([results_df, tomorrow_df], ignore_index=True)

    write_spine_to_bq(combined_df)
    print(f"✅ Daily spine update complete — {len(combined_df)} races written.")
