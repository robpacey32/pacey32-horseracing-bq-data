    ...
    for attempt in range(max_retries):
        try:
            ...
            for elem in rows:
                ...
                if m := re.search(r"Class\s+(\d+)", txt):
                    r_class = m.group(1)
                ...
                if m := re.search(r"Off time:\s*([0-9:]+)", txt):
                    off_time = m.group(1)
                if re.search(r"\d+\s*(m|f|y)", txt):
                    r_dist = txt
                if re.search(r"(Soft|Good|Firm|Heavy|Standard)", txt):
                    r_going = txt
                if re.search(r"(Turf|Tapeta|Polytrack|AW)", txt, re.I):
                    r_surface = txt
...
def main():
    ...
    for i, row in df.iterrows():
        ...
        # Preserve original abandoned reason
        prev = row["Status"]
